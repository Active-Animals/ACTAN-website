...
*Bear in mind that this ontology has NO COPYRIGHT, so neither will any of your contributions to it [(see Licensing page)](licences).*

Please see the footnote if you want to suggest edits to this website.[^1]
...
3. On your Projects list page, ensure that the 'Shared with Me' box is ticked. If you have been invited to the project, it should appear there as the latest version of 'Active Animals ALPHA'. You may need to refresh the page, or even log back in if the page has expired.
4. Open the project by clicking the project's name, or find more opening options in the burger button.
...
NB: The primordial version of the ontology has been developed as a unitary draft, without edited Commits, but from the ALPHA2 version, the original author also became subject to this expectation.

#### Inclusive Comments
Your early forays into collaborating may be in the form of Comments only. When you have editing privileges, still consider whether proposing a planned change in the form of a Comment might be the best way to start. (This is a WebProtégé View which does not directly enter the ontology structure.) Try to gain consent, if not consensus. Remember that your edits may be reverted if they appear controversial and have had no discussion. For new terms, make your comment against a parent class if you choose to wait before creating the new class. (Waiting may become the norm if new classes too often have to be reverted.)
...
Annotations allow us to understand more about a given class. It may give information about by whom and when it was created, its synonyms, or comments about it, etc. Annotations may have a single sub-annotation level containing similar information. Annotations are added to the ontology via Annotation Property dialogues.

#### DO Use
##### For Behaviours OR Dispositions
  - (Meaning:
    - Annotation property
  ...
  - Automatically introduced annotations for new terms are:
    - created_by
    - creation_date
  - Required annotations for all new terms are:
    - rdfs:label
      - dc:date (only the year)
      - dc:source OR
      - dc:contributor (i.e your details, if you have constructed a new label, but relate it to synonyms if you do; you should not *ad hoc* create terms without a related source)
    - hasDefinition
      - dc:source AND dc:contributor OR
      - rdfs:isDefinedBy (where a definition is substantially quoted even if not originally intended as such AND add a skos:scopeNote here quoting the whole sentence you have quoted from)
  - As required annotations are:
    ...
    - broadMatch (to link an external ontology term which is not equivalent, but fully includes your term)
    - closeMatch (to link an external ontology term which is almost equivalent to your term. You should not use direct equivalence or DbXref, since we should instead import and use the external term. NB: given that NBO is effectively defunct, no imports or direct equivalence terms should be used with it)
    - narrowMatch (to link an external ontology term which is not equivalent, but is fully included by your term)
    - relatedMatch (to link an external ontology term which significantly but incompletely overlaps with your term)

#### DON'T Use
  - A number of Annotation Properties belonging to external ontologies may have been retained for their broad utility, but please don't use them for our terms.
  ...
  - hasDbXref or Xref (see the list of possible matches to use instead above)
...
Bear in mind before adding the relation, that the class which has_ part another class must ALWAYS do so to be axiomatically correct. This is the inverse relation of [part_of](#part_of).
